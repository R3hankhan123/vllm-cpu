...
export CMAKE_ARGS="-DLLVM_ENABLE_PROJECTS=lld;libunwind;compiler-rt"
export CFLAGS="$(echo $CFLAGS | sed 's/-fno-plt //g')"
export CXXFLAGS="$(echo $CXXFLAGS | sed 's/-fno-plt //g')"
export CMAKE_ARGS="$CMAKE_ARGS -DFFI_INCLUDE_DIR=$PREFIX/include"
export CMAKE_ARGS="$CMAKE_ARGS -DFFI_LIBRARY_DIR=$PREFIX/lib"
cmake -DCMAKE_INSTALL_PREFIX="$PREFIX" \
    ...
    ${CMAKE_ARGS} -GNinja ../llvm && ninja install . 
cd ${CURDIR}
...
# -------------------------
microdnf install -y llvm-devel llvm-static
cd ${CURDIR}
